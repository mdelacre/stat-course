...
correlation_graph_c= "Figure 5.3"
correlation_graph_d= "Figure 5.4"
correlation_graph_e= "Figure 5.5"
correlation_graph_f= "Figure 5.6"
correlation_graph_g= "Figure 5.7"
correlation_graph_h= "Figure 5.8"
correlation_graph_i= "Figure 5.9"
correlation_graph_j= "Figure 5.10"
...
Les variables **quantitatives** sont les variables dont les valeurs sont intrinsèquement numériques, c'est-à-dire qu'il fait sens d'effectuer des opérations mathématiques sur leur valeurs. Par exemple, si un particulier a deux poules, que la première pond trois oufs et la deuxième en pond deux, il est correct de dire que le fermier trouvera 5 oufs lorsqu'il se rendra dans son poulailler. La variable "nombre d'oufs" est donc bien une variable quantitative.
...
```{r exercice1, echo=FALSE}
knitr::include_graphics("C:/Users/Marie/Documents/Github_projects/IPFC Compta/stat-course/Exercice 1, chapitre 2.PNG")
```
...
C'est ce qu'on appelle des données brutes. Une manière de résumer cet ensemble d'information est de calculer un **tableau de fréquences**, tel que représenté dans la `r fumeur_freq`. 
...
Il s'agit d'un tableau dans lequel chaque ligne représente une valeur possible de la variable. Comme vous pouvez l'observer, deux lignes ont été envisagées (une pour les fumeurs, une pour les non fumeurs) et le nombre de personnes appartenant à chaque catégorie constitue ce que l'on appelle la **fréquence absolue** ou l'**effectif** (ce sont deux synonymes). 

De même que pour les données brutes, nous prendrons l'habitude d'utiliser certaines notations. Dans un tableau de fréquence, on symbolise généralement l'ensemble des valeurs que peut prendre la variable par une lettre minuscule (par exemple, les différentes valeurs que peut prendre la variable X seront symbolisées par x). On utilise l'indice _j_ pour se référer à chaque valeur possible, j pouvant varier de 1 à k (dans le cas présent, k = `r length(names_fumeur)`). On symbolise par $x_j$ (x indicé j) la jème valeur possible de la variable X. Par exemple, si l'on choisit de nommer X la variable "fumer", $x_2=`r names_fumeur[2]`$. $n_j$ correspond aux fréquences absolues associées à chaque valeur $x_j$. Par exemple, $n_1$= `r abs_fumeur[1]`. 

La somme des fréquences absolues correspond au nombre total de personnes constituant l'échantillon (ici, n vaut `r sum(fumeur_tab$abs_fumeur)`).

$$n=\sum_{j=1}^k n_j = n_1+n_2=`r abs_fumeur[1]`+`r abs_fumeur[2]` = `r sum(abs_fumeur)`$$
Avec k étant le nombre de modalités de la variable étudiée.

Dans la formule ci-dessus, on voit apparaître pour la première fois du cours le symbole $\sum$. Il s'agit du symbole de sommation. $\sum_{j=1}^n n_j$ doit être lu comme ceci: "la somme de tous les $n_j$, avec j pouvant varier de 1 à k". Dans l'exemple de la variable "fumeur", étant donné que cette dernière contient deux modalités, j peut varier de 1 à 2. $\sum_{j=1}^n n_j$ est donc égal à $n_1+n_2$. 
...
Par exemple, `r abs_fumeur[1]` personnes de l'échantillon sur `r sum(fumeur_tab$abs_fumeur)` ne fument pas. La fréquence relative associée à la catégorie de non fumeurs se notera $f_1$ et vaudra `r abs_fumeur[1]`/`r sum(abs_fumeur)`= `r abs_fumeur[1]/sum(abs_fumeur)`. On peut également exprimer la fréquence relative en pourcentage:

$$f_j(pourcentage)=\frac {n_j}{n} X 100$$
Dans ce cas, on dira que `r abs_fumeur[1]`/`r sum(abs_fumeur)` x 100 = `r round(abs_fumeur[1]/sum(abs_fumeur)*100,2)` % des personnes de notre échantillon ne fument pas. 
...
L'ordre d'apparition des différents bâtonnets importe peu lorsqu'on représente des variables nominales. De plus, étant donné que l'axe des abscisses n'est pas normé, la largeur des bâtons importe peu également (même si pour le confort des yeux, on les gardera généralement uniformes). 

La `r barplot_fumeur_freq`  montre deux diagrammes en bâtons de notre distribution. Que l'on représente les fréquences absolues ou les fréquences relatives sur l'axe des ordonnées importe peu en termes de forme de graphe. En revanche,  l'information donnée est un petit peu différente: dans le premier cas, on peut dénombrer le nombre de fumeurs et de non fumeurs, dans le second cas, on peut déterminer le pourcentage de personnes qui fument ou ne fument pas dans l'échantillon.

`r barplot_fumeur_freq`  
_Diagramme en bâtonnets de la distribution de la `r fumeur_freq`_
...
`r abs_fumeur[1]` personnes ne fument pas (valeur $x_j$ = `r names_fumeur[1]`).  Cela représente `r abs_fumeur[1]/length(fumeur)*100`% de l'échantillon, l'angle correspondra donc à `r abs_fumeur[1]/length(fumeur)*100`% des 360°, à savoir  `r abs_fumeur[1]/length(fumeur)*360`

Bien que ce ne soit pas le cas dans cet exemple, les fréquences relatives contiendront parfois un nombre important (voire infini) de décimales. Il est dès lors généralement plus facile de déterminer les angles de chaque part sur base des fréquences absolues plutôt que sur base des fréquences relatives. La `r  fumeur_degres` fournit les angles qui devraient être associés à chaque catégorie de l'exemple de la `r fumeur_freq`:
...
data3.1_freq=data.frame(1:length(names_data3.1),names_data3.1,abs_data3.1,round(rel_data3.1,2),abscum_data3.1,round(relcum_data3.1,2)) 
```
...
La `r table3.1_freq` constitue le tableau des fréquences de ces données.

###### Pagebreak

`r table3.1_freq`  
_Transformation de la `r table3.1_brut` en tableau de fréquences_
```{r echo=FALSE, results='asis'}
kable(data3.1_freq,format="markdown",align="c",col.names=c("j","nombre d'enfants ($x_j$)","$n_j$","$f_j$ (en %)","$N_j$","$F_j$ (en %)"))
```
 
Comme vous pouvez l'observer, de même que fait précédemment pour les variables nominales, les nombres possibles d'enfants par ménage (ou modalités de la variable) ont été envisagés et on leur associe  une fréquence absolue ($n_j$), et une fréquence relative ($f_j$). Par ailleurs, la somme des fréquences absolues correspond toujours au nombre total de ménages constituant l'échantillon (ici, n vaut `r sum(data3.1_freq$abs_data3.1)`), et la somme des fréquences relatives correspond toujours à 1 (ou à 100%, si exprimée en pourcentage).
...
$$\sum_{j=1}^k f_j = `r chaine(rel_data3.1)` = `r sum(rel_data3.1)`$$ 

Contrairement au cas des variables nominales, l'ordre de présentation de chaque ligne a une importance. On triera les valeurs possibles de la variable représentée par ordre croissant (`r names_data3.1[1]` apparaitra avant `r names_data3.1[2]`, `r names_data3.1[2]` apparaitra avant `r names_data3.1[3]`, etc.). 
...
$$N_j=\sum_{j=1}^j n_j,  1 \le j \le k$$

Cela permet de déterminer rapidement, par exemple, que `r abscum_data3.1[4]` ménages sont constitués de maximum `r names_data3.1[4]` enfants. Enfin,  cette information peut être exprimée par rapport à l'ensemble des ménages. On dira alors que `r round(relcum_data3.1[4],2)` % des ménages sont constitués de maximum `r names_data3.1[4]` enfants. Cette dernière expression correspond aux  **fréquences relatives cumulées**, soit la dernière colonne du tableau (notation: $F_j$). 
...
pie(table(data3.1_brut$Enfant),labels=abs_data3.1,cex.main=.9,cex.lab=.9,main="Nombre d'enfants par ménages",col=col_pie)
...
La `r  data3.1_degres` fournit les angles qui devraient être associés à chaque catégorie dans l'exemple de la `r table3.1_freq`. Ceux-ci ont été déterminés en procédant à une règle de trois, comme expliqué dans la section dédiée aux diagrammes circulaires pour représenter des variables nominales. Essayez de déterminer ces valeurs par vous-même, afin de vous assurer que vous avez bien compris le principe. 
...
Il a déjà été précisé que ce type de graphique est peu précis et que lorsque deux catégories sont associées à des fréquences relativement similaires, il devient très difficile de repérer leur différence. Dans la `r pie_data3.1_freq`, les deux premières catégories semblent de taille égale, alors qu'elles ne le sont pas. Il était nettement plus aisé de le voir dans la `r barplot_data3.1_freq`. C'est dû au fait que notre oeil repère plus facilement une différence de hauteur entre deux objets alignés (deux rectangles du diagramme en bâtons) qu'une différence d'aire dans un cercle. Pour cette raison, je vous recommanderai de privilégier autant que possible le diagramme en bâtons. La simplicité est la clé d'un bon graphique!
...
Il arrive que l'on parle de "diagramme en escalier", et que l'on choisisse une représentation plus proche de celle de la `r stair_data3.1_freq`, dans laquelle un point noir indique la fréquence associée  à une catégorie en particulier. L'information fournie par les diagrammes en escalier et les diagrammes en bâtons des fréquences cumulées est identique.
...
Enfin, le **centre de classe** est la valeur qui se trouve à égale distance de la borne inférieure et de la borne supérieure d'une classe. Par exemple, le centre de la première classe de la `r table3.2_freq`  vaut `r as.character(data3.2_freq$centre_PIB[1])`.
Connaître cette valeur servira à estimer les **quantiles** de variables continues (ce concept sera très prochainement expliqué). 
...
`r table3.3_freq_a`  
_Table de fréquence de catégories d'âges d'amplitudes égales dans un échantillon de `r length(Age)` personnes_
```{r echo=FALSE, results='asis'}
...
`r table3.3_freq_b`  
_Table de fréquence de catégories d'âges d'amplitudes inégales dans un échantillon de `r length(Age)` personnes_
```{r echo=FALSE, results='asis'}
...
Les classes de valeurs sont représentées en abscisse. Lorsque toutes les classes sont de taille égale, la hauteur des rectangles est proportionnelle aux fréquences (absolues ou relatives; les fréquences correspondent à l'ordonnée), tel qu'on peut le voir dans la `r barplot_data3.2_freq` ainsi que dans la `r barplot_data3.3_freq_a`   .  
...
$h_i=\frac{n_i}{a_i}$ avec $a_{j}$ = amplitude de la classe.
...
hist(data3.3_brut$Age,breakpoints <- c(0, 3,12,18,65,110),xaxt="n",col="lightgrey",main="Histogramme de l'âge",ylab="hi",cex.lab=1,xlab="Age")
...
_Histogramme de la distribution de la `r table3.2_freq` et polygone des fréquences_
...
_Histogramme de la distribution de la `r table3.3_freq_a` et polygone des fréquences_ 
...
De même que pour les histogrammes vu dans la section précédente, il est possible de tracer le polygone des fréquences cumulées pour mieux rendre compte du caractère continu de la variable. 
...
```{r med6, echo=FALSE,include=FALSE}
...
Soit une série statistique contenant un nombre impair de sujets (n=`r length(serie1)`): 
...
Le rang médian vaut $\frac{`r length(serie3)`+1}{2}=`r (length(serie3)+1)/2`$. La médiane sera donc comprise entre les valeurs`r  serie3_ordered[floor((length(serie3)+1)/2)]`) et `r serie3_ordered[ceiling((length(serie3)+1)/2)]`, et vaudra donc: `r (serie3_ordered[floor((length(serie3)+1)/2)]+serie3_ordered[ceiling((length(serie3)+1)/2)])/2`. 

Pour chaque série, intéressons nous uniquement aux valeurs qui se trouvent à gauche de la médiane. Le premier quartile d'une série sera la valeur telle que la moitié des observations sélectionnées lui est inférieure et l'autre moitié lui est supérieure. Similairement, si l'on s'intéresse uniquement aux valeurs à droite de la médiane pour chaque série, le troisième quartile d'une sera sera la valeur telle que la moitié des observations sélectionnées lui est inférieure et l'autre moitié lui est supérieure.
...
La valeur du premier quartile sera donc égale à `r quartiles(serie1)[[1]][1]`.

* La partie à droite de la médiane est la partie suivante: `r quartiles(serie1)[[3]]`  

La valeur du troisième quartile sera donc égale à `r quartiles(serie1)[[1]][3]`.
...
La valeur du premier quartile sera donc égale à `r quartiles(serie2)[[1]][1]`.

* La partie à droite de la médiane est la partie suivante: `r quartiles(serie2)[[3]]`  

La valeur du troisième quartile sera donc égale à `r quartiles(serie2)[[1]][3]`.
...
La valeur du premier quartile sera donc égale à `r quartiles(serie3)[[1]][1]`.

* La partie à droite de la médiane est la partie suivante: `r quartiles(serie3)[[3]]`  

La valeur du troisième quartile sera donc égale à `r quartiles(serie3)[[1]][3]`.
...
La fréquence cumulée associée à la valeur `r max(as.numeric(names_serie1[relcum_serie1<=.5]))` vaut `r relcum_serie1[names_serie1==max(as.numeric(names_serie1[relcum_serie1<=.5]))]`. Dans la mesure où la médiane est la valeur telle qu'il y ait 50% des observations à gauche de celle-ci, elle sera nécessairement supérieure à `r max(as.numeric(names_serie1[relcum_serie1<=.5]))`.  La fréquence cumulée associée à la valeur `r min(as.numeric(names_serie1[relcum_serie1>=.5]))` vaut `r relcum_serie1[names_serie1==min(as.numeric(names_serie1[relcum_serie1>=.5]))]`. La médiane vaudra donc `r min(as.numeric(names_serie1[relcum_serie1>=.5]))`.  Vous pourriez vous dire que `r relcum_serie1[names_serie1==min(as.numeric(names_serie1[relcum_serie1>=.5]))]*100`% dépasse 50%. N'oubliez cependant pas que nous étudions ici des séries de données discrètes. Autrement dit, il n'y a pas de valeur intermédiaire possible entre `r max(as.numeric(names_serie1[relcum_serie1<=.5]))` et `r min(as.numeric(names_serie1[relcum_serie1>.5]))`. Vérifiez la valeur de la médiane de la série 1 au départ des séries brutes, et vous verrez qu'elle vaut bien `r min(as.numeric(names_serie1[relcum_serie1>=.5]))`. 
...
`r boxplot_serie1`  
_Boîte à moustaches représentant les données de la `r table3.7`_
```{r boxplot_data3.7, echo=FALSE}
par(mar=c(1,2,1,1))
boxplot(data3.7_brut,ylim=c(0,20),col="lightblue",xaxt="n",yaxt="n",bty="n")
axis(2,at=seq(0,20,2),labels=seq(0,20,2))
```
...
La fréquence cumulée au centre de classe `r as.character(centre_PIB[1])` vaut `r round(relcum_data3.2[1],2)` (soit une valeur inférieure à .5 ou 50%). La fréquence cumulée  de la valeur `r as.character(centre_PIB[2])` vaut `r round(relcum_data3.2[2],2)`% (soit une valeur supérieure à .5 ou 50%). La valeur de la médiane se situera donc entre `r as.character(centre_PIB[1])` et `r as.character(centre_PIB[2])` et sera telle qu'il y ait exactement 50% des observations à sa gauche, et 50% des observations à sa droite. Une simple règle de trois permet de déterminer sa valeur: il y a une distance de `r round(relcum_data3.2[2],2)- round(relcum_data3.2[1],2)` unités entre `r round(relcum_data3.2[2],2)`% et `r round(relcum_data3.2[1],2)`%. Sur l'axe des x, cette distance est projetée sur une distance de `r as.character(centre_PIB[2]-centre_PIB[1])` (soit l'écart entre les deux centres de classes). 

Il y a une distance de `r 50 - round(relcum_data3.2[1],2)` unités entre 50% et `r round(relcum_data3.2[1],2)`%. Par règle de trois, on en déduit que cela correspond à une distance de $\frac{`r as.character(centre_PIB[2]-centre_PIB[1])`}{`r round(relcum_data3.2[2],2)- round(relcum_data3.2[1],2)`} \times `r 50- round(relcum_data3.2[1],2)` = `r (centre_PIB[2]-centre_PIB[1])/(round(relcum_data3.2[2],2)- round(relcum_data3.2[1],2))*(50- round(relcum_data3.2[1],2))`$ sur l'axe des x. Par conséquent, la médiane sera estimée à `r as.character(centre_PIB[1])`+`r (centre_PIB[2]-centre_PIB[1])/(round(relcum_data3.2[2],2)- round(relcum_data3.2[1],2))*(50- round(relcum_data3.2[1],2))` = `r as.character(centre_PIB[1]+(centre_PIB[2]-centre_PIB[1])/(round(relcum_data3.2[2],2)- round(relcum_data3.2[1],2))*(50- round(relcum_data3.2[1],2)))`. 

En suivant un raisonnement identique, on pourra déterminer la valeur de n'importe quel quantile. Le troisième quartile, par exemple, sera tel qu'il y ait exactement 75% des données à sa gauche et 25% à sa droite. La fréquence cumulée au centre de classe `r as.character(centre_PIB[2])` vaut `r round(relcum_data3.2[2],2)`% (soit une valeur inférieure à .75 ou 75%). La fréquence cumulée  de la valeur `r as.character(centre_PIB[3])` vaut `r round(relcum_data3.2[3],2)`% (soit une valeur supérieure à .75 ou 75%). La valeur du troisième quartile se situera donc entre `r as.character(centre_PIB[2])` et `r as.character(centre_PIB[3])` et sera telle qu'il y ait exactement 75% des observations à sa gauche, et 25% des observations à sa droite. Une simple règle de trois permet de déterminer sa valeur: il y a une distance de `r round(relcum_data3.2[3],2)- round(relcum_data3.2[2],2)` unités entre `r round(relcum_data3.2[3],2)`% et `r round(relcum_data3.2[2],2)`%. Sur l'axe des x, cette distance est projetée sur une distance de `r as.character(centre_PIB[3]-centre_PIB[2])` (soit l'écart entre les deux centres de classes). 

Il y a une distance de `r 75 - round(relcum_data3.2[2],2)` unités entre 75% et `r round(relcum_data3.2[2],2)`%. Par règle de trois, on en déduit que cela correspond à une distance de $\frac{`r as.character(centre_PIB[3]-centre_PIB[2])`}{`r round(relcum_data3.2[3],2)- round(relcum_data3.2[2],2)`} \times `r 75- round(relcum_data3.2[2],2)` = `r (centre_PIB[3]-centre_PIB[2])/(relcum_data3.2[3]- relcum_data3.2[2])*(75-relcum_data3.2[2])`$ sur l'axe des x. Par conséquent, le troisième quartile sera estimé à `r as.character(centre_PIB[2])`+`r (centre_PIB[3]-centre_PIB[2])/(round(relcum_data3.2[3],2)- round(relcum_data3.2[2],2))*(75- round(relcum_data3.2[2],2))` = `r round(centre_PIB[2]+(centre_PIB[3]-centre_PIB[2])/(relcum_data3.2[3]- relcum_data3.2[2])*(75- relcum_data3.2[2]),2)`. 
...
knitr::include_graphics("C:/Users/Marie/Documents/Github_projects/IPFC Compta/Synt_graphiques.PNG")
```

## Exercices de fin de chapitre

```{r echo=FALSE, results='asis'}
statut=c("veuf","divorcé","divorcé","célibataire","séparé","divorcé","marié","veuf","célibataire","divorcé","divorcé","marié","séparé","séparé","veuf","marié","célibataire","divorcé","marié","divorcé","divorcé","célibataire","divorcé","célibataire","divorcé","veuf","séparé","marié","divorcé","célibataire","marié","divorcé","veuf","marié","célibataire","séparé","célibataire","veuf","séparé","marié","célibataire","marié","célibataire","célibataire")
id=1:length(statut)
Table=data.frame(id,statut)

# découpe en vue de l'affichage graphique
affich_col=4
Num_gr=matrix(0,length(statut)/affich_col,affich_col) 
Statut_gr=matrix(0,length(statut)/affich_col,affich_col)
affich_ex=matrix(0,length(statut)/affich_col,affich_col*2)

for(j in 1:affich_col){
  Num_gr[,j]=((length(statut)/affich_col)*(j-1)+1):(length(statut)/affich_col*j)
  Statut_gr[,j]=statut[((length(statut)/affich_col)*(j-1)+1):(length(statut)/affich_col*j)]
  affich_ex[,((3*j)-(j+1)):(2*j)]=cbind(Num_gr[,j],Statut_gr[,j])
  }
```

1. Pour `r length(statut)` domiciliée dans le brabant wallon, on dispose d'une information sur le statut matrimonial. 

```{r echo=FALSE, results='asis'}
kable(affich_ex,format="markdown",align="c",col.names=rep(c("i","Statut matrimonial ($X_i$)"),affich_col))
```
...
```{r echo=FALSE, results='asis'}
km=c(17,18,19,20,22,22,23,23,24,25,25,26,26,27,27,27,27,27,28,28,28,28,28,29,29,29,29,30,30,30,30,30,31,31,31,31,31,32,32,33,33,34,35,35,36,36,38,39,40,41)
id=1:length(age)

Table=data.frame(age,id)

# découpe en vue de l'affichage graphique
affich_col=5
Num_gr=matrix(0,length(age)/affich_col,affich_col) 
age_gr=matrix(0,length(age)/affich_col,affich_col)
affich_ex2=matrix(0,length(age)/affich_col,affich_col*2)

for(j in 1:affich_col){
  Num_gr[,j]=((length(age)/affich_col)*(j-1)+1):(length(age)/affich_col*j)
  age_gr[,j]=age[((length(age)/affich_col)*(j-1)+1):(length(age)/affich_col*j)]
  affich_ex2[,((3*j)-(j+1)):(2*j)]=cbind(Num_gr[,j],age_gr[,j])
  }
```

```{r echo=FALSE, results='asis'}
kable(affich_ex2,format="markdown",align="c",col.names=rep(c("i","age ($Y_i$)"),affich_col))
```
...
2.2. Dessinez l'histogramme et le polygone des fréquences de cette distribution observée.  
2.3. Dessinez l'histogramme des fréquences cumulées, et le polygone des fréquences cumulées.  
...
```{r echo=FALSE, results='asis'}
age=c(rep(0,80),rep(1,95),rep(2,125),rep(3,75),rep(4,60),rep(5,65))
Table=table(age)
kable(Table,col.names=c("age ($x_{j}$)","$n_{j}$"))
```
...
```{r,include=TRUE, echo=FALSE, results='asis'}
QI=c(69.5,107.51,77.02,99.63,108.89,97.03,113.38,99.61,90.29,109.7,93.49,126.59,99.73,112.79,103.08,54.88,79.51,93.64,103.55,64.86,114.43,90.93,88.71,76.67,78.19,100.84,107.64,68.53,84.93,108.04,93.2,132.48,118.69,108.93,100.07,104.19,89.41,109.42,122.2,116.25,87.8,75.72,98.36,106.61,120.26,80.22,105.47,103.5,117.91,99.58,101,103)
Groupe=c(rep("A",13),rep("B",13),rep("C",13),rep("D",13))
data_QI=data.frame(QI,Groupe)
```

_Boîte à moustaches représentant le QI de 4 groupes: A, B, C et D_
```{r "boxplot exercice", echo=FALSE}
par(mar=c(2,2,2,2))
boxplot(QI~Groupe)
```
...
rel_data4.1=abs_data4.1/sum(abs_data4.1)
data4.1_freq=data.frame(as.character(names_data4.1),as.character(abs_data4.1),round(rel_data4.1,2))
colnames(data4.1_freq)=c("$y_j$","$n_j$","$f_j$")
```
 
_**(b) Tableau de fréquence**_  
```{r echo=FALSE, results='asis'}
kable(t(data4.1_freq),rownames="",align="c",)
```

Pour rappel, on utilise une lettre majuscule indicée par _i_ pour décrire les données brutes ($Y_i$ = la valeur du ième sujet sur la variable Y), et une lettre minuscule indicée par _j_ lorsqu'on parle des valeurs de la variable Y dans un tableau de fréquence ($y_j$ = la jème valeur que peut prendre la variable Y).
...
rel_data4.2=abs_data4.2/sum(abs_data4.2)
data4.2_freq=data.frame(as.character(names_data4.2),as.character(abs_data4.2),round(rel_data4.2,2))
colnames(data4.2_freq)=c("$y_j$","$n_j$","$f_j$")
```

Le mode correspond à la classe la plus représentée. Dans la `r table4.1` (a ou b), vous constaterez aisément que `r names_data4.1[which(abs_data4.1 == max(abs_data4.1))]` est le nombre d'années d'ancienneté le plus représenté, puisque 
...
En appliquant ces formes à l'ancienneté fournie dans la `r table4.1`, on obtient ceci: 
...
Dans l'exemple des notes de cours de la `r notes_compta`, on trouve que l'étendue des notes en français vaut `r max(notes_brut$francais)`-`r min(notes_brut$francais)`=`r max(notes_brut$francais)-min(notes_brut$francais)`, et que celle des notes en mathématiques vaut `r max(notes_brut$math)`-`r min(notes_brut$math)`= `r max(notes_brut$math)-min(notes_brut$math)`. 
...
Dans l'exemple des notes de cours de la `r notes_compta`, on trouve que l'écart interquartile des notes en français vaut `r francais_Q$quartiles[3]`-`r francais_Q$quartiles[1]`=`r francais_Q$quartiles[3]-francais_Q$quartiles[1]`, et que celui des notes en mathématiques vaut `r math_Q$quartiles[3]`-`r math_Q$quartiles[1]`= `r math_Q$quartiles[3]-math_Q$quartiles[1]`. 
...
```{r med2, echo=FALSE,include=FALSE}
ex4.1=c(6,1,3,15,6,12,3,13,15,15)
serie1_ordered=sort(ex4.1)
ex4.2=c(9,12,15,8,75)
serie2_ordered=sort(ex4.1)
```
...
```{r med3, echo=FALSE,include=FALSE}
ex4.3=c(rep(7,6),rep(8,4),rep(9,5),rep(10,9),rep(11,6))
name=names(table(ex4.3))
tab=tabulate(ex4.3)
freq=tab[min(as.numeric(name[1])):length(tab)]
kable(data.frame(name,freq,round(freq/sum(tab)*100,2)),format="markdown",align="c",col.names=c("j","$n_j$","$f_j$"))
```
...
```{r med4, echo=FALSE,include=FALSE}
n=10000000
data1=rchisq(n,5)
data2=rnorm(n,0,2)
data3=1-rchisq(n,12)
...
4. Voici trois distributions. L'une des trois distributions a une valeur G2 (asymétrie) de `r round(skewness(data1),2)`. Une autre a une valeur G2 de `r round(skewness(data3),2)`et enfin, la dernière a une valeur G2 de `r round(skewness(data2),2)`. Veuillez déterminer quelle valeur se rapporte à chaque distribution.

```{r med5, echo=FALSE,include=FALSE}
par(mfrow=c(1,3))
plot(density(data1),xlab="",main="Série A")
plot(density(data2),xlab="",main="Série B")
plot(density(data3),xlab="",main="Série C")
```
...
Nous avons déjà déterminé précédemment dans quelle mesure les scores de chacun des cours variaient d'un individu à l'autre (nous avions trouvé, respectivement pour les cours de mathématiques et de français, des variances de 
`r var(math)*(length(math)-1)/length(math)` et `r var(francais)*(length(francais)-1)/length(francais)` et donc par conséquent, des écart-types de respectivement `r round(sd(math)*(length(math)-1)/length(math))` et `r round(sd(francais)*(length(francais)-1)/length(francais),2)`). On se demande à présent s'il existe un **lien** entre la manière dont ces deux cours varient simultanément d'un sujet à l'autre. Autrement dit, on se demande si ces deux variables **covarient**.
...
On constate que l'ensemble des points semblent se rapprocher d'une droite croissante. Cela signifie que le score des deux variables tend à varier dans le même sens: lorsque la note au cours de mathématiques augmente, il en est de même pour la note au cours de français. On parle alors de relation linéaire **positive**.

Attention: il s'agit d'une **tendance** des données. Autrement dit, ce n'est pas nécessairement vrai pour chaque point du nuage. Regardez les deux points entourés dans la reproduction du  nuage de points ci-dessous. Le point le plus à gauche dans le cercle représente un étudiant ayant obtenu une note de `r math[2]` en mathématiques, et une note de `r francais[2]` en français. Le point le plus à droite dans le cercle représente un étudiant ayant obtenu une note de `r math[3]` en mathématiques, soit une note plus élevée que le premier sujet. Pourtant, la note qu'il a obtenu en français est plus faible (`r francais[3]`). Lorsque les points sont parfaitement alignés, on parole de relation linéaire **parfaite**. 

`r correlation_graph_c`  
_Nuage de points représentant la relation entre la note à un cours de math et la note à un cours de français_  
```{r echo=FALSE, results='asis'}
par(mar=c(4,4,2,4))
plot(math,francais,pch=19,bty="n",xlim=c(0,10),ylim=c(0,10))
...
id = 1:10
math=c(8.5,4,4.5,6,3.5,6.5,7,5,7.5,7.5)
geo=c(0.5,7,6.5,6,6.5,2.5,3,5,2.5,2.5)
notes_geo=data.frame(id,math,geo)
```

`r notes_compta2`  
_Notes (/10) obtenues par `r length(id)` des étudiants en mathématiques et en géographie_
```{r echo=FALSE, results='asis'}
...
```{r echo=FALSE, results='asis'} 
id = 1:10
phys=c(5.5,9,6,6,6.5,6.5,8,8.5,8.5,9)
anglais=c(10,5,6,7,5,8,7,9,8,9)
notes_phys=data.frame(id,phys,anglais)
```

`r notes_compta2`  
_Notes (/10) obtenues par `r length(id)` des étudiants en éducation physique et en anglais_
```{r echo=FALSE, results='asis'}
kable(notes_phys,format="markdown",align="c",col.names=c("$i$","education physique($V_i)$","anglais($V_i)$")) 
```

Contrairement aux deux situations précédentes, il ne semble pas possible de résumer correctement le nuage de points par une droite croissante ou décroissante. 
Il ne semble pas y avoir de relation linéaire entre les notes obtenues au cours de géographiques et celles obtenues au cours d'histoire. 
...
`r correlation_graph_h`
_Reproduction du nuage de points représentant la relation entre la note à un cours de math et la note à un cours de géographie, découpé en 4 quadrants_  
```{r echo=FALSE, results='asis'}

par(mar=c(4,4,2,4))
plot(math,geo,pch=19,bty="n",xlim=c(3,10),ylim=c(0,8))
axis(1,at=seq(3,10,1),labels=seq(3,10,1))
axis(2,at=seq(0,8,1),labels=seq(0,8,1))
abline(v=mean(math))
abline(h=mean(geo))
```

Comme dernier exemple, nous avions déterminé précédemment graphiquement une absence de relation linéaire entre les cours d'éducation physique et celui d'anglais. Cela signifiait que lorsque la note au cours d'éducation physique augmentait, celle du cours d'anglais pouvait tantôt augmenter, tantôt diminuer. Graphiquement, cela amène à observer un nuage de points avec des points qui se répartissent aléatoirement dans les 4 quadrants (voir la `r correlation_graph_i` ). 
 
 `r correlation_graph_i`
_Reproduction du nuage de points représentant la relation entre la note à un cours de géographie et la note à un cours d'anglais, découpé en 4 quadrants_  
```{r echo=FALSE, results='asis'}
par(mar=c(4,4,2,4))
plot(phys,anglais,pch=19,bty="n",xlim=c(5,10),ylim=c(4,10))
axis(1,at=seq(5,10,1),labels=seq(5,10,1))
axis(2,at=seq(4,10,1),labels=seq(4,10,1))
abline(v=mean(phys))
abline(h=mean(anglais))
```

Il a déjà été déterminé que pour les quadrants inférieur gauche et supérieur droit, le produit $(X-\bar{X})(Y-\bar{Y})$ était positif, et qu'au contraire, pour les quadrants supérieur gauche et inférieur droit, le produit était négatif. Cela signifie que le terme $\sum_{i=1}^n$ consistera à additionner toute une série de nombres dont certais sont positifs et d'autres sont négatifs, et par voie de conséquence, que la mesure de covariance sera proche de zéro (puisque les positifs et les négatifs s'annulent). 
...
$$r_{XY} = \frac{S_{XY}}{S_XS_Y} = \frac{[(X-\bar{X})(Y-\bar{Y})]/n}{S_XS_Y} $$
...
plot(A,B,main="linéaire positive",xlab="parfaite",ylab="")
plot(A,D,main="linéaire nulle",xlab="",ylab="")
plot(A,E,main="linéaire négative",xlab="parfaite",ylab="")
plot(A,C,main="",xlab="non parfaite",ylab="")
plot(0,0,col="white",xaxt="n",yaxt="n",xlab="",ylab="",bty="n")
plot(A,F,main="",xlab="non parfaite",ylab="")
```
...
```{r echo=FALSE, results='asis'}
n=length(math)
math_centre=math-mean(math)
francais_centre=francais-mean(francais)
geo_centre=geo-mean(geo)
phys_centre=phys-mean(phys)
agls_centre=anglais-mean(anglais)

sd_math=sqrt(sum(math_centre^2)/length(math))
sd_francais=sqrt(sum(francais_centre^2)/length(francais))
sd_geo=sqrt(sum(geo_centre^2)/length(geo))
sd_phys=sqrt(sum(phys_centre^2)/length(phys))
sd_agls=sqrt(sum(agls_centre^2)/length(anglais))

covar_math_francais=sum((math_centre)*(francais_centre))/n
covar_math_geo=sum((math_centre)*(geo_centre))/n
covar_phys_agls=sum((phys_centre)*(agls_centre))/n

correl_math_francais=covar_math_francais/(sd_math*sd_francais)
correl_math_geo=covar_math_geo/(sd_math*sd_geo)
correl_phys_agls=covar_phys_agls/(sd_phys*sd_agls)
```

```{r echo=FALSE, results='asis'}
interpretation=function(rel){
  if (rel==0){
  interpretation="corrélation nulle"
  } else if (rel>(-.3) & rel <.3 & rel !=0){
  interpretation="corrélation faible"  
  } else if (rel<(-.5)) {
  interpretation="corrélation forte"  
  } else if (rel >.5){
  interpretation="corrélation forte" 
  } else {interpretation = "corrélation moyenne"}
  return(interpretation)
}
```

A titre d'exercice, vérifiez que $S_{Math,Francais}=`r round(covar_math_francais,2)`$, $S_{Math,Geographie}=`r round(correl_math_geo,2)`$ et $S_{Education physique,Anglais}= `r round(correl_phys_agls,2)`$.

En transformant ces mesures de covariances en mesure de corrélation, on trouve que la corrélation entre les cours de mathématiques et de français vaut $r_{Math,Francais}=\frac{`r round(covar_math_francais,2)`}{`r round(sd_math,2)` X `r round(sd_francais,2)`}=`r round(correl_math_francais,2)`$, soit une `r interpretation(correl_math_francais)`

La corrélation entre les cours de mathématiques et de géographie vaut $r_{Math,Geo}= \frac{`r round(covar_math_geo,2)`}{`r round(sd_math,2)` X `r round(sd_geo,2)`}=`r round(correl_math_geo,2)`$, soit une `r interpretation(correl_math_geo)`

Enfin, la corrélation entre les cours d'éducation physique et d'anglais vaut $r_{phys,anglais}=\frac {`r round(covar_phys_agls,2)`}{`r round(sd_phys,2)` X `r round(sd_agls,2)`}=`r round(correl_phys_agls,3)`$, soit une `r interpretation(correl_phys_agls)` (voir même très faible, pratiquement nulle). 
...
```{r echo=FALSE, results='asis'}
x=seq(-50,50,.1) 
y=-x^2+3+rnorm(length(x),0,100)
plot(x,y,pch=19,cex=.5,xaxt="n",xlim=c(-60,60),xlab="Quantité de caféine (en ml)",ylab="niveau d'énergie",ylim=c(-3000,500),yaxt="n")
axis(2, at = seq(-3000,500,500), labels = seq(1,8,1), las = 1)
axis(1, at = seq(-60,60,20), labels = seq(0,600,100), las = 1)
```
...
```{r echo=FALSE, results='asis'}
math=c(75,38,49,80,71,65,68,20,90,52)
eco=c(100,48,68,95,87,92,80,24,83,64)
plot(math,eco,pch=19,cex=.5)
```

3.1. Représenter graphiquement la relation entre les deux variables.
3.2. Calculez la covariance entre les cours de mathématiques et d'économie.
3.3. Calculez la corrélation entre les cours de mathématiques et d'économies.
