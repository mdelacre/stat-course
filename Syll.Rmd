...
Les variables **quantitatives** sont les variables dont les valeurs sont intrinsèquement numériques, c'est-à-dire qu'il fait sens d'effectuer des opérations mathématiques sur leur valeurs. Par exemple, si un particulier a deux poules, que la première pond trois oeufs et la deuxième en pond deux, il est correct de dire que le fermier trouvera 5 oeufs lorsqu'il se rendra dans son poulailler. La variable "nombre d'oeufs" est donc bien une variable quantitative.
...
Il s'agit d'un tableau dans lequel chaque ligne représente une valeur possible de la variable. Comme vous pouvez l'observer, deux lignes ont été envisagées (une pour les fumeurs, une pour les non fumeurs) et le nombre de personnes appartenant à chaque catégorie constitue ce que l'on appelle la **fréquence absolue**. 
...
L'ordre d'apparition des différents bâtonnets importe peu lorsqu'on représente des variables nominales. De plus, étant donné que l'axe des abcisses n'est pas normé, la largeur des bâtons importe peu également (même si pour le confort des yeux, on les gardera généralement uniformes). 
...
`r abs_fumeur[1]` personnes ne fument pas (valeur $x_j$ = `r names_fumeur[1]`).  Cela représente `r abs_fumeur[1]/length(fumeur)*100`% de l'échantillon, l'angle correspondra donc à `r abs_fumeur[1]/length(fumeur)*100`% des 360°. 
...
Contrairement au cas des variables nominales, l'ordre de présentation de chaque ligne a une importance. On triera les valeurs possibles de la variable représentée par ordre croissant (`r names_data3.1[1]` apparait avant `r names_data3.1[2]`, `r names_data3.1[2]` apparait avant `r names_data3.1[3]`, etc.). 
...
Il a déjà été précisé que ce type de graphique est peu précis et que lorsque deux catégories sont associées à des fréquences relativement similaires, il devient très difficile de repérer leur différence. Dans la `r pie_data3.1_freq`, les deux premières catégories semblent de taille égale, alors qu'elles ne le sont pas. Il était nettement plus aisé de le voir dans la `r barplot_data3.1_freq`. C'est dû au fait que notre oeil repère plus facilement une différence d'hauteur entre deux objets alignés (deux rectangles du diagramme en bâtons) qu'une différence d'aire dans un cercle. Pour cette raison, je vous recommanderai de privilégier autant que possible le diagramme en bâtons. La simplicité est la clé d'un bon graphique!
...
Bien entendu, les trois formules donnent une valeur identique de la moyenne (bien sûr, à condition d'utiliesr des valeurs non arrondies pour les fréquences relatives). C'est parce qu'il s'agit de trois manières différentes de calculer la même chose. 
...
Dans l'exemple des notes de cours de la `r notes_compta`, on trouve que l'étendue des notes en français vaut `r max(notes_brut$francais)`-`r min(notes_brut$francais)`=`r max(notes_brut$francais)-min(notes_brut$francais)`, et que celle des notes en mathématiques vaut ``r max(notes_brut$math)`-`r max(notes_brut$math)`= `r max(notes_brut$math)-min(notes_brut$math)`. 
...
La variance et l'écart-type souffrent de la même limite que la moyenne: ils sont tous les deux sensibles aux valeurs abberrantes. Cette sensibilité est accentuée par le fait que l'on élève les écarts au carré. Ces mesures restent malgré tout les mesures de dispersion les plus utilisées en pratique. 
...
Jusque là, nous avons passé beaucoup de temps à étudier comment il était possible de représenter graphiquement ou algébriquement les variables prises isolément. Le dernier objectif de ce cours est d'étudier comment deux variables peuvent varier **en même temps**, ou **covarier**. Cela relève de la statistique **bivariée** (par opposition à statistique **univariée**, lorsqu'on les étudie séparément les unes des autres).

Revenons à l'exemple de la `r notes_compta`  dans laquelle nous avions étudié la note de 10 étudiants à deux cours: le cours de mathématiques et le cours de français. 
...
Nous avons déjà précédemment déterminé dans quelle mesure les scores de chacun des cours variaient d'un individu à l'autre (nous avions trouvé, respectivement pour les cours de mathématiques et de français, des variances de 
`r var(math)*(length(math)-1)/length(math)` et `r var(francais)*(length(francais)-1)/length(francais)` et donc par conséquent, des écart-types de respectivement `r sd(math)*(length(math)-1)/length(math)` et `r sd(francais)*(length(francais)-1)/length(francais)`). On se demande à présent s'il existe un **lien** entre la manière dont ces deux cours varient d'un sujet à l'autre. Autrement dit, on se demande si ces deux variables **covarient**.
...
Attention: il s'agit d'une **tendance** des données. Autrement dit, ce n'est pas nécessairement vrai pour chaque point du nuage. Regardez les deux points entourés dans la reproduction du  nuage de points ci-dessous. Le point le plus à gauche dans le cercle représente un étudiant ayant obtenu une note de `r math[2]` en mathématiques, et une note de `r francais[2]` en français. Le point le plus à droite dans le cercle représente un étudiant ayant obtenu une note de `r math[3]` en mathématiques, soit une note plus élevée que le premier sujet. Pourtant, la note qu'il a obtenu en français est plus faible, puisqu'elle ne vaut que `r francais[3]`. Le seul cas pour lequel on pourra affirmer avec certitude que la relation est vrai pour chaque point du nuage sera celui où l'entièreté des points seront parfaitement alignés (on parlera alors de relation linéaire positive **parfaite**).
...
`r notes_compta2`  
_Notes (/10) obtenues par `r length(id)` des étudiants en géographie et en anglais_
```{r echo=FALSE, results='asis'}
kable(notes_phys,format="markdown",align="c",col.names=c("$i$","education physique($V_i)$","anglais($V_i)$")) 
```

Contrairement au deux situations précédentes, il ne semble pas possible de résumer correctement le nuage de points par une droite croissante ou décroissante. 
...
Si l'on essaye malgré tout de résumer le nuage de points par la droite qui se rapproche le plus près possible de tous les points, on trouve une droite de pente pratiquement nulle. C'est ce qui arrive en absence de **relation linéaire** entre deux variables.
...
Enfin, la corrélation entre les cours d'éducation physique et d'anglais vaut $r_{phys,anglais}=\frac {`r round(covar_phys_agls,2)`}{`r round(sd_phys,2)` X `r round(sd_agls,2)`}=`r round(correl_phys_agls,2)`$, soit une`r interpretation(correl_phys_agls)` (voir même très faible, pratiquement nulle). 
